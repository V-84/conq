...
Reproduces §1 of `queue-warden-implementation-spec-FINAL.md`. Three arms hit a fake
...
- **A** — `p-queue@9` (strict) with `p-retry` wrapped inside each task. Retries
  ...
  large task loss.
- **B** — a `queue-warden`-style queue: every attempt (first or retry) re-acquires a
...
- **C** — stock `p-queue`, no `p-retry`; retries are manually re-enqueued via
  ...
  "retries must re-enter admission control," not "use queue-warden."

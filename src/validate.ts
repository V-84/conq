...
export function validatePositiveIntOrInfinity(name: string, value: unknown): number {
  ...
  if (typeof value !== 'number' || !Number.isInteger(value) || value < 1) {
    throw new TypeError(
      `queue-warden: '${name}' must be a positive integer or Infinity, received ${describe(value)}`,
    );
  }
  return value;
}
...
export function validatePositiveInt(name: string, value: unknown): number {
  if (typeof value !== 'number' || !Number.isInteger(value) || value < 1) {
    throw new TypeError(
      `queue-warden: '${name}' must be a positive integer, received ${describe(value)}`,
    );
  }
  return value;
}
...
export function validateNonNegativeInt(name: string, value: unknown): number {
  if (typeof value !== 'number' || !Number.isInteger(value) || value < 0) {
    throw new TypeError(
      `queue-warden: '${name}' must be a non-negative integer, received ${describe(value)}`,
    );
  }
  return value;
}
...
export function validateNonNegativeFinite(name: string, value: unknown): number {
  if (typeof value !== 'number' || !Number.isFinite(value) || value < 0) {
    throw new TypeError(
      `queue-warden: '${name}' must be a non-negative finite number, received ${describe(value)}`,
    );
  }
  return value;
}
...
export function validateFactor(name: string, value: unknown): number {
  if (typeof value !== 'number' || !Number.isFinite(value) || value < 1) {
    throw new TypeError(
      `queue-warden: '${name}' must be a finite number >= 1, received ${describe(value)}`,
    );
  }
  return value;
}
...
function throwInputPromise(index: number): never {
  throw new TypeError(
    `queue-warden: input[${index}] is a Promise, not a value.\nPassing already-created Promises means the work has already started, so queue-warden\ncannot bound concurrency or rate. Pass plain items and do the work in the worker:\n  queue-warden.mapConcurrent(items, (item) => doWork(item))`,
  );
}

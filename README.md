...
[![CI](https://github.com/V-84/qfence/actions/workflows/ci.yml/badge.svg)](https://github.com/V-84/qfence/actions/workflows/ci.yml)
[![codecov](https://codecov.io/gh/V-84/qfence/graph/badge.svg?token=a6764b50-c5c4-4a96-b798-2029ba9b6a3d)](https://codecov.io/gh/V-84/qfence)
[![npm](https://img.shields.io/npm/v/qfence)](https://www.npmjs.com/package/qfence)
...
// broken: every fetch has already started before qfence sees it
...
// -> TypeError: qfence: input[0] is a Promise, not a value.
...
```js
import { mapConcurrent } from 'qfence';
...
`qfence` makes retries re-enter admission control by default, so this cannot happen.
...
admission control,” not “use qfence.” Their perfect zero-429 zero-latency result
...
`qfence` gives you on top of that: correct-by-default retry, first-error abort
...
```ts
import { forEachConcurrent } from 'qfence';
...
```ts
import { TimeoutError, AbortError } from 'qfence';
...
- **Retry holds its concurrency slot** across the backoff delay, but
  **re-acquires a rate-limit token** before the next attempt. This is the core
  of what `qfence` does differently.
...
| Feature | `Promise.all` | `p-limit` | `p-map` | `p-queue` | **qfence** |
|---|---|---|---|---|---|
...
Everything above the bold rows is table stakes `qfence` matches. The bold rows
are the four things that differentiate it.

## When you should not use qfence
...
- You need persistence, dead-letter queues, or distributed queuing. That is
  BullMQ / SQS / RabbitMQ territory. `qfence` is in-process only.
- You need worker threads or real parallelism. `qfence` is cooperative I/O
  concurrency.
